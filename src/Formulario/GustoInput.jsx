...
const GustoInput = ({ index, gusto, cantidad, handleGustoChange }) => {
  return (
    <div className="gusto-input">
      ...
      <input type="number" className="u-full-width" value={cantidad} onChange={(e) => handleGustoChange(index, 'cantidad', e.target.value)} min="1" required />
      </div>
    </div>
  );
};
